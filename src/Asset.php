...
class Asset {
	...
	public static function addScript( $data = array() ) {
		self::add( 'script', $data );
	}

	public static function addStyle( $data = array() ) {
		self::add( 'style', $data );
	}
		...
		foreach ( self::$data['script'] as $data ) {
			...
			if ( $data['ajax'] ) {
				$params = array(
					...
					'ajax_nonce' => wp_create_nonce( $data['name'] ),
				);
	...
	protected static function set_params( $type, $data ) {
		...
		$data['deps']    = isset( $data['deps'] ) ? $data['deps'] : array();
	...
	protected static function get_processed_files() {
		...
		self::$files = get_option( 'wp-register-files', array() );
	}
	...
	protected static function set_new_params( $type, $hash, $url, $extension ) {
		...
		self::$data[ $type ] = array( $data['name'] => $data );
